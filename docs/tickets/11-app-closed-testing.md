...
## Notes（缺陷收斂・2026-08-13）

- 封閉測試整包尚未開工（仍 `ready-for-agent`）；本日為前置手動驗證與阻擋級 UI 缺陷收斂。
- **已驗證**：02／08／10／16；09 行為＋「已達分析上限」文案。
- **已修**：L00／L09 動畫黑塊（淺色 wash）；首頁 Tab 強制回 L05（Shell 絕對路由）；韓文硬閘文案；注音橫排；語言>5 下拉。
- **進行中缺陷**：阿語列表 `AutomationId may only be set one time`（見票 13 Notes）——未修畢前勿當封閉測試綠燈。
