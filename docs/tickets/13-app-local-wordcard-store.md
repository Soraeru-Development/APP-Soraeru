...
- [x] 登出或刪除帳號後，本機單字本依 ADR-0007 語意處理：**明確登出／401／換帳保留多使用者本機列**；刪帳只清該 OwnerUserId（見 Notes；2026-08-26 SQLite）。
...
- App 存／列表／刪改走 `LocalNotebookService`＋`SqliteLocalWordCardStore`（`FileSystem.AppDataDirectory/local-wordcards.db`；若有舊 `local-wordcards.json` 會一次性遷移）。  
- 刪除為本機 soft-delete（`DeletedAtUtc`／`UpdatedAtUtc`）以預留票 14；**明確登出／401／換帳不清他帳列**（只清 session 或改過濾；同帳重登保留 SoT）。刪帳只清當前 `OwnerUserId` 列。登入後會 `SyncAsync`（Login 略過 Splash）。
- **刪帳（ADR-0007 Q5=A／Q9=A）**：Settings「刪除帳號」→ `DELETE /api/v1/me`（清該使用者雲端鏡像 WordCards＋Users 列）→ 成功或 401 後 `ClearLocalNotebookAsync`（**僅該使用者列**）＋清 session → Login。決策邏輯在 `SessionAuthGate`。
- **審查補洞（2026-08-11）**：
  - Splash：本機有 token 且 API 不可達 → 進已登入離線流（依本機 onboarding flag）；**不**丟 Login。
  - Splash／Settings：GetMe 回傳 null（401/403）→ 清 session 回 Login（**2026-08-26 起不清本機 SoT**，保留多使用者列）；連線失敗不清庫。
...
- **煙測建議**：登入→離線重開 App 應進 Home／Onboarding 且可本機存刪；模擬 401 應清 session 並回 Login（本機他帳／同帳卡仍在）；設定頁刪帳後該帳雲端／本機列空且回 Login；結果頁對同字再存應開既有卡且個人空耳不變（改空耳用詳情編修）。
...
- **殘餘**：離線時無法驗證 JWT 是否已過期（僅有 token＋網路失敗仍會進離線寫）；連回線後 GetMe 401 才清 session。刪帳不刪 UsageDaily 孤兒列（無 FK；完整法遵清檔可後做）。
...
- **2026-08-21 修復／調查**：
  ...
  - **當時調查**：本機單字卡 SoT 曾為 **JSON**（簡化）；**2026-08-26 使用者更正**：改為 **SQLite** 多使用者並存。
- **2026-08-24 登出／重登：** 明確登出**不清**本機 SoT（`SessionAuthGate.DecideLogout`；同帳重登保留）；Login／Register 後 `SyncAsync`；`NotebookListRefreshGate`＋`AppShell.ResetNotebookListPage` 避免 Shell 分頁快取導致登入後第一時間列表空白。票狀態仍 **done**（本輪為缺陷修復，非重開 AC）。
- **2026-08-26 多使用者 SQLite SoT：** `SqliteLocalWordCardStore`＋DI；換帳／401 不再整庫清空；`ClearLocalNotebookAsync` 只刪當前使用者列；JSON→SQLite 一次性遷移。