...
| **11／列表** | 語言 chip→picker | 單字本語言選項（不含全部）>5 改下拉；注音直立排已修 |
| **13** | ⚠️ 阿語列表 WIP | `AutomationId may only be set one time`；已加除錯未修畢；斷網步驟見票 Notes |
| **10** | ✅ 已驗證 | 隱私權政策、AI 內容聲明頁有內容 |
| **16** | ✅ 已驗證 | 可自行修改諧音（個人空耳） |
| **04** | 金標待策展建立 | 學習者不能自建；目前庫空則無「聽感已核定」屬預期（見票 Notes） |
| **07** | ✅ 混合 OCR 已接 | ML Kit 多腳本＋Tesseract tessdata_fast；實機品質待驗；**中期亞洲語言包僅討論、未落地** |
...
| [07](07-app-ocr-select-one.md) | App MVP | 裝置端 OCR 選一字進分析 | done | MediaPicker＋**ML Kit 多腳本＋Tesseract** 混合 OCR＋單選解析 | 已跑-有開放項 | 自動+手動（實機 OCR 品質） | — | 實機驗證日／泰／中等腳本 | **語言包約 37 MB**；多腳本序掃耗時；實機品質待驗；中期亞洲包僅討論未落地 | 0.5h（08-11）＋0.5h 規劃（08-13） |
...
| [11](11-app-closed-testing.md) | App 上架 | 封閉測試就緒與缺陷收斂 | **ready-for-agent** | 07–10 全 done；前置手動驗證／UI 缺陷收斂中；封閉測試整包尚未開工 | 已跑-有開放項 | 手動（§15 檢核表；封閉測試整包） | 07–10（皆 done） | 產測試建置＋回歸清單；先清阿語 AutomationId | Spec 指出 AC 未勾、封閉測試整包待執行；13–18 驗收需對齊 ADR-0007 | 0.5h（08-13） |
| [12](12-app-play-store-submit.md) | App 上架 | 商店素材與送審 | **blocked** | 等 11 封閉測試通過 | 不適用 | 手動（Play Console） | 11 | 11 完成後準備素材 | — | — |
| [13](13-app-local-wordcard-store.md) | Client-first | 本機單字卡儲存與列表／存／刪 | done | `LocalNotebookService` 本機 SoT；刪帳／session 清庫 | 未跑 | 自動+手動（煙測） | — | 修阿語 AutomationId；離線煙測見 Notes | **阿語列表 AutomationId WIP（08-13）**；離線 JWT 過期偵測；UsageDaily 孤兒列 | ~4.25h（08-11）＋1.5h（08-13） |
...
| [18](18-app-local-notebook-lookup-short-circuit.md) | Client-first | 同字再查本機短路＋詳情重新分析 | done | ClientLogic Gate＋App 短路／重新分析已交付；code-review-dual 通過 | 已跑-有開放項 | 自動+手動（本機短路／重新分析煙測） | 13（done） | 封閉輪次勾選煙測 | 他機未同步卡仍會分析（ADR-0008 接受）；列表缺陷見 13 | 1.5h（08-12） |
...
- **封閉測試（11）**：blocker 07–10 已全 done；前置缺陷收斂進行中；**阿語 AutomationId WIP** 未清前勿當綠燈；封閉測試整包尚待執行。  
- **Client-first 串**：13–18 全 done；**16 手動已驗證**；13 斷網步驟見票 Notes；**阿語列表 WIP** 見 13。  
- **04 金標**：待策展以 API 建立啟用條目後再驗 App verified 標示。  
- **07 OCR**：已改 **ML Kit 多腳本＋Tesseract tessdata_fast** 混合（圖不上雲）；實機品質待驗；中期亞洲語言包**僅討論未落地**。  
