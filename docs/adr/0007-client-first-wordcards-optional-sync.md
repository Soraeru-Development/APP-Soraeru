...
| 合併 | 卡 ID 聯集＋上列規則；**換帳以 OwnerUserId 過濾**（同裝置多帳號列並存，不清他帳） |
| 刪除／刪帳 | tombstone 同步；刪帳刪該使用者雲端單字本＋**僅該 OwnerUserId 本機列**；登出／401 **不清**本機他帳資料 |
| 本機 SoT 引擎 | **SQLite**（`local-wordcards.db`）；既有 `local-wordcards.json` 一次性遷移 |
| 個人空耳編輯 | 詳情頁隨時可改 |
| 雲端隱私 | 伺服器可讀；匯出可後做 |
...
- **本機引擎（2026-08-26 修正）**：SoT 為裝置端 **SQLite**（非單一共享 JSON 檔）。多使用者列以 `OwnerUserId` 並存；登出／401／換帳只改 session／列表過濾，**不得**整庫清空他帳卡片。刪帳僅清當前擁有者列。可自 `local-wordcards.json` 一次性遷移。  
